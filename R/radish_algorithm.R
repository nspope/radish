...
# experimental, updating Cholesky using inbuild Matrix method
# doesn't seem like there's an advantage to this
radish_algorithm3 <- function(f, g, s, S, theta = rep(0, ncol(s$x)), objective = TRUE, gradient = TRUE, hessian = TRUE, partial = TRUE, nonnegative = TRUE, validate = FALSE)
{
  stopifnot(    class(f) == "radish_conductance_model")
  stopifnot(    class(g) == "radish_measurement_model")
  stopifnot(    class(s) == "radish_graph3"            )
  stopifnot(    class(S) == "matrix"                  )
  stopifnot(class(theta) == "numeric"                 )
  ...
  v    <- sqrt(ones / N)
  Z    <- Matrix::Diagonal(N)[,s$demes]
  Zn   <- as.matrix(In %*% Z - (In %*% v) %*% (t(v) %*% Z)) #cnvt to dense
  LQn  <- Matrix::update(s$choleski, Qn) #Cholesky(Qn, LDL = TRUE)
  G    <- as.matrix(Matrix::solve(LQn, Zn)) #cnvt to dense
  E    <- t(Zn) %*% G 
